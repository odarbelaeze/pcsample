...
class Sample(object):
    ...
    def y_param(self):
        ...
        """
        return 2.0 * np.sin(np.degrees(120)) * self.macro_lattice_parameter()
    ...
    def regular_lattice(self):
        ...
        xtics = np.arange(0, self.x_max(), self.a)
        ytics = np.arange(0, self.y_max(), a)
    ...
    def in_circles(self, positions):
        """
        Return the portion of the sample inside the circles.
        """
        if self.in_rand:
            positions = random_lattice()
        else:
            positions = regular_lattice()

        mask = utils.in_circles_mask(positions, self.centers(), self.rad)
        return positions[mask]

    def out_circles(self, positions):
        """
        ...
        mask = utils.out_circles_mask(positions, self.centers(), self.rad)
        return positions[mask]
